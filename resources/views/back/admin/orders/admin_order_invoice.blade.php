...
<body>
  ...
  <table width="100%" style="background: #F7F7F7; padding:0 20px 0 20px;">
    <tr>
        ...
        <td align="right">
          Order Status: <span class="badge rounded-pill bg-danger">{{ $order->status }}</span>
            <pre class="font" >
        ...
        <td>
          <p class="font" style="margin-left: 20px;">
           ...
           <strong>Phone:</strong> {{ $order->phone }} <br>
            @php
            $reg =  $order->region->region_name; 
            $dis =  $order->district->district_name;
            $city = $order->city->city; 
            @endphp
           <strong>Address:</strong> {{ $order->adress }} / {{$reg}} / {{ $dis }}/ {{ $city }}<br>
        ...
        <td>
          <p class="font">
            ...
            Order Date: {{ $order->order_date }} <br>
             Delivery Date: {{ $order->delivered_date }}
