...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $allowedStatusIdentities = [];
        if ($siteSetting && ($siteSetting->show_status_student ?? true)) $allowedStatusIdentities[] = 'student';
        if ($siteSetting && ($siteSetting->show_status_non_student ?? false)) $allowedStatusIdentities[] = 'non-student';
        if ($siteSetting && ($siteSetting->show_status_partner ?? true)) $allowedStatusIdentities[] = 'partner';
        if (empty($allowedStatusIdentities)) $allowedStatusIdentities = ['student', 'non-student', 'partner'];

        $request->validate([
            'name' => ['required', 'string', 'max:255'],
            'email' => ['required', 'string', 'email', 'max:255', 'unique:'.User::class],
            'status_identity' => ['required', 'string', 'in:' . implode(',', $allowedStatusIdentities)],
        ...
        if ($referrer) {
            // Referral commissions are earned upon the customer's first purchase via cumulative tiers
